<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @elseif(Auth::user()->rolSistema->nombre === 'estudiante')
                        ...
                        <x-nav-link :href="route('estudiante.stats.dashboard')" :active="request()->routeIs('estudiante.stats.*')">
                            {{ __('Mi Progreso') }}
                        </x-nav-link>
                
                        <x-nav-link :href="route('estudiante.habilidades.index')" :active="request()->routeIs('estudiante.habilidades.*')">
                            {{ __('Mis Habilidades') }}
                        </x-nav-link>
                        
                        <x-nav-link :href="route('estudiante.eventos.index')" :active="request()->routeIs('estudiante.eventos.*')">
                            {{ __('Eventos') }}
                        </x-nav-link>
                        
                        <x-nav-link :href="route('estudiante.equipo.index')" :active="request()->routeIs('estudiante.equipo.*')">
                            {{ __('Mi Equipo') }}
                        </x-nav-link>
                    @endif
                </div>
                    ...
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            ...
                            <img src="{{ Auth::user()->foto_perfil_url }}" 
                                 ...
                                 class="w-8 h-8 rounded-full object-cover border-2 border-gray-300 me-2">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @elseif(Auth::user()->rolSistema->nombre === 'estudiante')
                <x-responsive-nav-link :href="route('estudiante.eventos.index')" :active="request()->routeIs('estudiante.eventos.*')">
                    {{ __('Eventos') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('estudiante.equipo.index')" :active="request()->routeIs('estudiante.equipo.*')">
                    {{ __('Mi Equipo') }}
                </x-responsive-nav-link>
            @endif
        </div>
